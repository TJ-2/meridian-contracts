...
contract USMTaxRewards is ILQTYStaking, Ownable, CheckContract, BaseMath {
    using SafeMath for uint;

    // --- Data ---
    string constant public NAME = "USMTaxRewards";
    ...
    event StakingGainsWithdrawn(address indexed staker, uint LUSDGain); // event StakingGainsWithdrawn(address indexed staker, uint LUSDGain, uint ETHGain);
    // event F_ETHUpdated(uint _F_ETH);
    event F_LUSDUpdated(uint _F_LUSD);
    event TotalLQTYStakedUpdated(uint _totalLQTYStaked);
    // event EtherSent(address _account, uint _amount);
    event StakerSnapshotsUpdated(address _staker, uint _F_LUSD); // event StakerSnapshotsUpdated(address _staker, uint _F_ETH, uint _F_LUSD);
    ...
    function stake(uint _LQTYamount) external override {
       ...
       _updateUserSnapshots(msg.sender);
        ...
        emit StakingGainsWithdrawn(msg.sender, LUSDGain);  //  emit StakingGainsWithdrawn(msg.sender, LUSDGain, ETHGain);
    ...
    */}
    ...
    function _updateUserSnapshots(address _user) internal {
        ...
        emit StakerSnapshotsUpdated(_user, F_LUSD); // emit StakerSnapshotsUpdated(_user, F_ETH, F_LUSD);
